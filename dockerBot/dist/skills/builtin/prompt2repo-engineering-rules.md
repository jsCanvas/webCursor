---
name: prompt2repo-engineering-rules
description: Prompt2Repo unified engineering rules for Docker-first full-stack delivery, compose networking, ORM, logging, UI, README, and registry mirrors. Use when scaffolding apps, reviewing delivery quality, or when the user mentions Prompt2Repo, 交付规范, or 容器化验收.
---
...
> 本文件由仓库根目录 `user_rule.md` 同步为 dockerBot **内置技能**；在聊天中启用技能 `prompt2repo-engineering-rules` 后注入 Agent。原稿中的 `trigger: always_on` 由用户在客户端勾选本技能等效实现。
